...
const StoryModel = ({setShowModel,fetchStories}) => {
  ...
  const handleMediaUpload=(e)=>{
    ...
    if(file){
      setMedia(file);
      setPreviewUrl(URL.createObjectURL(file)); //tempoiray url for frontend
    }
  }
    ...
    <div className="fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4" >
      <div className='w-full max-w-md'>
          ...
          )}
          {
            mode==="media"&&previewUrl&&(
              media?.type.startsWith("image")?(
                ...
                <video src={privewUrl}  className="object-contain max-h-full"/>
              )
            )
          }
        ...
        <div className="flex  gap-2 mt-4">
          ...
          <label className={`flex-1 flex items-center justify-center gap-2 p-2 cursor-pointer rounded cursor-pointer ${mode==='media'?"bg-white text-black" :"bg-zinc-800"}`}>
        ...
        })} className="flex items-center justify-center gap-2 text-white py-3 mt-4 w-full rounded bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hovwer:to-purple-700 active:scale-95 transition cursor-pointer">
